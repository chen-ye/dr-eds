...
This repo now contain open source WEB/PWA app for EDS OX/TX which is hosted at https://dreds.jeckyll.net/.
...
- FD, RD thresholds - need more research actual functions of there - for now it is known only that RD treshold=1 is race mode 2 is normal mode. FD also have treshold, but it is uknown for what is used.
...
Make sure `logs` button in the menu is pressed and debug messages are visible, then douple tap on them and they will be copied to the clipboard. Open new issue here and paste debug information, so they can be analized and problem to be fixed.
