...
function log(s)
{
	...
	// GeX and OX2 spam with multiple messages when gear change which app to lag, so update logs only when appState.debug is shown via menu button
	if (appState.show_debug) {
	if ((appState.device_type != "EDS OX2") && (appState.device_type != "EDS GeX"))
	    $('.debug').append(ts + '> ' + s + '\n');
	    $('.debug').scrollTop($('.debug').prop("scrollHeight"));
	}

	appState.debug_log.push(ts + '> ' + s);
...
function handleCharacteristicValueChanged(event)
{
	...
	var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_startRead, 0x00, 0x00, 0x00 ]);
	a = setCRC16(a);
	...
	appState.blocks = r.payload[1];
	appState.current_block = 0;

	startBlock(lang['L_GET_INFO'] + " " + (appState.current_block + 1) + '/' + appState.blocks, 0);
	// let's read all the data
	var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_read, 0x03, 0x00, appState.current_block, 0x51, 0x00, 0x00 ]);
	a = setCRC16(a);
	characteristic_TX.writeValueWithoutResponse(a);
	log("Send: read -> block " + appState.current_block);
    } else
	...
	appState.info['NUM'] = r.payload[4];
	var t = parseInt(r.payload[4]);
	t = parseInt(appState.info['TOTAL_CNT']) - t + 1;
	if (r.payload[5] == 1)
	...
	$('.settings .action_buttons .button.gears select, .ox2settings .action_buttons .button.gears select, .txsettings .action_buttons .button.gears select, .gxsettings .action_buttons .button.gears select').val(parseInt(appState.info['TOTAL_CNT']));//.change();
	$('.live .gear').html(t);
	$('.live .gears').html('/' + appState.info['TOTAL_CNT']);
	...
	if ((r.payload[4] > 0) && (r.payload[4] < 7)) {
	    appState.info['Q_NUM'] = r.payload[4];
	    if ((appState.info['Q_NUM'] == 1) || (appState.info['Q_NUM'] == 2) || (appState.info['Q_NUM'] == 3)) t = 1;
	    if ((appState.info['Q_NUM'] == 4) || (appState.info['Q_NUM'] == 5) || (appState.info['Q_NUM'] == 6)) t = 2;
	...
	endBlock();
	if (
	    (appState.device_type == "EDS OX") ||
	    (appState.device_type == "EDS OX2")
	) {
	    if (r.payload[0] == 0x00) {
		clearTimeout(appState.ctimeout);
	...
	if (appState.all_gears.length == 0) {
	    endBlock();
	}
	if (r.payload[0] == 0x00) {
	    clearTimeout(appState.ctimeout);

	    // iterate on all remaining items into appState.all_gears until all are gone
	    if (appState.all_gears.length > 0) {
		...
		var prefix = "";
		if (appState.device_type == "EDS TX") prefix = "tx";
		if (appState.device_type == "EDS GeX") prefix = "gx";
		if (appState.device_type == "EDS OX2") prefix = "ox2";
		...
		if ((appState.single_set > 0) && (tg - appState.single_set + 1 == cg)) {
			...
			var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_rearLifting, 0x01, f1, 0x00, 0x00 ]);
			a = setCRC16(a);
			characteristic_TX.writeValueWithoutResponse(a);
			log("Send: rearLifting -> " + f1.toString(16).padStart(2, '0'));
			appState.chain = function() {
			    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_rearLifting, 0x01, f2, 0x00, 0x00 ]);
			    a = setCRC16(a);
			    characteristic_TX.writeValueWithoutResponse(a);
	...
	appState.info['H_POWER'] = r.payload[0] * 256 + r.payload[1];
	appState.info['R_POWER'] = r.payload[2] * 256 + r.payload[3];
	appState.info['Q_POWER'] = r.payload[6] * 256 + r.payload[7];
	appState.info['L_POWER'] = r.payload[8] * 256 + r.payload[9];
	...
	// check RD protection
	if (appState.first_call) {
	    appState.first_call = 0;

	    var f = 0x01;
	    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_backSetting1p, 0x01, f, 0x00, 0x00 ]);
	    a = setCRC16(a);
	    characteristic_TX.writeValueWithoutResponse(a);
	...
	if ($('.txsettings .fvnone').is(":visible")) {
		...
		if ((parseInt(r.payload[6]) != 0) && (parseInt(r.payload[7]) != 0)) {
		    ...
		    var f2 = 0x01;
		    var f3 = 0x06;
		    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_getFrontAndRearDerailleurGearValuesInfo, 0x03, f1, f2, f3, 0x00, 0x00 ]);
		    a = setCRC16(a);
		    characteristic_TX.writeValueWithoutResponse(a);
...
function parsePacket(hex, check_confirm = 0)
{
	...
	log("block " + hex[3] + ": " + s);
	appState.raw_info.push(s);

	appState.current_block++;
	if (appState.current_block < appState.blocks) {
	    startBlock(lang['L_GET_INFO'] + " " + appState.current_block + '/' + appState.blocks, 0);
	    ...
	    appState.raw_info = appState.raw_info.join('');
	    var a = appState.raw_info.split("\n");
	    for (var t = 0; t < a.length; t++) {
		if (a[t].substr(0, 8) == "MCU_DATA") {
		    var b = a[t].split(',');
		    var c = b[0].split(':');
		    appState.info[c[0]] = c[1] + ',' + b[1] + ',' + b[2] + ',' + b[3] + ',' + b[4];
		    var c = b[5].split(':');
		    appState.info[c[0]] = c[1] + ',' + b[6] + ',' + b[7];
		} else {
		    var b = a[t].split(",");
		    for (var r = 0; r < b.length; r++) {
			if (b[r] != "") {
			    var c = b[r].split(':');
			    appState.info[c[0]] = c[1];
			}
		    }
		}
	    }
	    ...
	    if ((appState.device_type == "EDS TX") || (appState.device_type == "EDS GeX") || (appState.device_type == "EDS OX2"))
		$('.button_shutdown').show();
	    $('.button_export').show();
	    $('.button_import').show();

	    appState.show_page = _getItem('appState.show_page');
	    // set appState.show_page by default
	    if (appState.show_page == null) appState.show_page = 'settings';
	    ...
	    var t = parseInt(appState.info['NUM']);
	    t = parseInt(appState.info['TOTAL_CNT']) - t + 1;
	    $('.settings .action_buttons .button.gear, .ox2settings .action_buttons .button.gear, .txsettings .action_buttons .button.gear, .gxsettings .action_buttons .button.gear').html(t);
	    $('.settings .action_buttons .button.gears select, .ox2settings .action_buttons .button.gears select, .txsettings .action_buttons .button.gears select, .gxsettings .action_buttons .button.gears select').val(parseInt(appState.info['TOTAL_CNT']));//.change();
	    $('.live .gear').html(t);
		...
		if (parseInt(appState.info['Q_TOTAL']) == 0) {
		    var f1 = 0x01;
		    var f2 = 0x01;
		    var f3 = 0x06;
		    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_getFrontAndRearDerailleurGearValuesInfo, 0x03, f1, f2, f3, 0x00, 0x00 ]);
		...
		if (parseInt(appState.info['KeySwitch']) == 0) {
		    $('.settings .buttons_function .vbutton:first-child').addClass('up');
	    ...
	    if (appState.device_type == "EDS OX2") {
		//$('.settings .buttons_function .button').removeClass('selected');
		if (appState.info['KeySwitch'].substring(1, 2) == 2) {
		    $('.settings .buttons_function .vbutton:first-child').addClass('up');
	    	    $('.settings .buttons_function .vbutton.top').html(lang['L_UP']);
	    	    $('.settings .buttons_function .vbutton.bottom').html(lang['L_DOWN']);
		} else {
	    ...
	    if (appState.device_type == "EDS TX") {
		if (parseInt(appState.info['KeySwitch'].substring(0, 1)) == 1) $('.txsettings .buttons_function .vbutton.small select').val('up');//lang['L_UP'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(0, 1)) == 2) $('.txsettings .buttons_function .vbutton.small select').val('down');//lang['L_DOWN'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(0, 1)) == 3) $('.txsettings .buttons_function .vbutton.small select').val('front');//lang['L_FRONT'].toLowerCase()

		if (parseInt(appState.info['KeySwitch'].substring(1, 2)) == 1) $('.txsettings .buttons_function .vbutton.big select').val('up');//lang['L_UP'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(1, 2)) == 2) $('.txsettings .buttons_function .vbutton.big select').val('down');//lang['L_DOWN'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(1, 2)) == 3) $('.txsettings .buttons_function .vbutton.big select').val('front');//lang['L_FRONT'].toLowerCase()

		if (parseInt(appState.info['KeySwitch'].substring(2, 3)) == 1) $('.txsettings .buttons_function .button.single select').val('up');//lang['L_UP'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(2, 3)) == 2) $('.txsettings .buttons_function .button.single select').val('down');//lang['L_DOWN'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(2, 3)) == 3) $('.txsettings .buttons_function .button.single select').val('front');//lang['L_FRONT'].toLowerCase()
	    } else
	    if (appState.device_type == "EDS GeX") {
		if (parseInt(appState.info['KeySwitch'].substring(0, 1)) == 1) $('.gxsettings .buttons_function .vbutton.small select').val('up');//lang['L_UP'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(0, 1)) == 2) $('.gxsettings .buttons_function .vbutton.small select').val('down');//lang['L_DOWN'].toLowerCase()

		if (parseInt(appState.info['KeySwitch'].substring(1, 2)) == 1) $('.gxsettings .buttons_function .vbutton.big select').val('up');//lang['L_UP'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(1, 2)) == 2) $('.gxsettings .buttons_function .vbutton.big select').val('down');//lang['L_DOWN'].toLowerCase()

		if (parseInt(appState.info['KeySwitch'].substring(2, 3)) == 1) $('.gxsettings .buttons_function .button.single select').val('up');//lang['L_UP'].toLowerCase()
		if (parseInt(appState.info['KeySwitch'].substring(2, 3)) == 2) $('.gxsettings .buttons_function .button.single select').val('down');//lang['L_DOWN'].toLowerCase()
	    }

	    if ((appState.device_type == "EDS TX") || (appState.device_type == "EDS GeX") || (appState.device_type == "EDS OX2")) {
		// send getTransmissionVersionInfo to get sleep status
		var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_getTransmissionVersionInfo, 0x00, 0x00, 0x00 ]);
		a = setCRC16(a);
		characteristic_TX.writeValueWithoutResponse(a);
...
function buildPresets()
{
    ...
    if (gears[appState.device_type] == null) return;

    var s = '';
    for (var t in gears[appState.device_type]) {
	...
	try {
	    ...
	    if (a[appState.device_type] != null) {
		for (var t in a[appState.device_type][$(this).attr('preset')]['rear'])
		{
			$('.' + pr + 'settings .gear_values .vcontent .content .gear[gear="' + a[appState.device_type][$(this).attr('preset')]['rear'][t].gear + '"] input').val(a[appState.device_type][$(this).attr('preset')]['rear'][t].value);
		}
		if (appState.device_type == "EDS TX") {
		    for (var t in a[appState.device_type][$(this).attr('preset')]['front'])
			$('.txsettings .gear_values .fvcontent .content .front[front="' + a[appState.device_type][$(this).attr('preset')]['front'][t].gear + '"] input').val(a[appState.device_type][$(this).attr('preset')]['front'][t].value);
		}
	    }
	...
	var pname =  $(this).attr('preset');
       ...
       _dialog(lang['L_PRESET_CONFIRM_DELETE'].replace('%s', pname), '', function(e) {
           ...
           try {
               var a = JSON.parse(_getItem('gears'));
               if (a[appState.device_type] != null)
                   delete a[appState.device_type][pname];
	...
	/*if (confirm(lang['L_PRESET_CONFIRM_DELETE'].replace('%s', pname))) {
		...
		var a = JSON.parse(_getItem('gears'));
		if (a[appState.device_type] != null)
		    delete a[appState.device_type][$(this).attr('preset')];
...
function updateBattery()
{
    ...
    if (appState.device_type == "EDS OX") {
	$('.info .content .left .left_ver').html(appState.info['GEARS_V']);
	if (b == 'percent') {
	    $('.info .content').attr('type', 'percent');
	    $('.info .content .left .left_val').html(percentage(appState.info['POWER_2']) + '%');
	} else {
	    $('.info .content').attr('type', 'volts');
	    $('.info .content .left .left_val').html((parseInt(appState.info['POWER_2']) / 100).toFixed(2) + 'V');
	}

	$('.info .content .right .right_ver').html(appState.info['REMOTE_V']);
	$('.info .content .right .right_val').html((parseInt(appState.info['POWER_1']) / 100).toFixed(2) + 'V');
    } else
    if (appState.device_type == "EDS OX2") {
	$('.info .ox2content .left .left_ver').html(appState.info['H_Ver']);
	if (b == 'percent') {
	    $('.info .ox2content').attr('type', 'percent');
	    $('.info .ox2content .left .left_val').html(percentage(appState.info['H_POWER']) + '%');
	} else {
	    $('.info .ox2content').attr('type', 'volts');
	    $('.info .ox2content .left .left_val').html((parseInt(appState.info['H_POWER']) / 100).toFixed(2) + 'V');
	}

	$('.info .ox2content .right .right_ver').html(appState.info['R_Ver']);
	$('.info .ox2content .right .right_val').html((parseInt(appState.info['R_POWER']) / 100).toFixed(2) + 'V');
    }
    if (appState.device_type == "EDS TX") {
        $('.info .txcontent .left .left_rver').html(appState.info['H_Ver']);
        $('.info .txcontent .left .left_fver').html(appState.info['Q_Ver']);
	if (b == 'percent') {
	    $('.info .txcontent').attr('type', 'percent');

	    $('.info .txcontent .left .left_fval').html(percentage(appState.info['Q_POWER']) + '%');
	    $('.info .txcontent .left .left_rval').html(percentage(appState.info['H_POWER']) + '%');
	} else {
	    $('.info .txcontent').attr('type', 'volts');

	    $('.info .txcontent .left .left_fval').html((parseInt(appState.info['Q_POWER']) / 100).toFixed(2) + 'V');
	    $('.info .txcontent .left .left_rval').html((parseInt(appState.info['H_POWER']) / 100).toFixed(2) + 'V');

	    if (b == null) _setItem('battery', 'volts');
	}

	$('.info .txcontent .right .right_lver').html(appState.info['L_Ver']);
	$('.info .txcontent .right .right_rver').html(appState.info['R_Ver']);
	$('.info .txcontent .right .right_lval').html((parseInt(appState.info['L_POWER']) / 100).toFixed(2) + 'V');
	$('.info .txcontent .right .right_rval').html((parseInt(appState.info['R_POWER']) / 100).toFixed(2) + 'V');
    } else
    if (appState.device_type == "EDS GeX") {
        $('.info .gxcontent .left .left_rver').html(appState.info['H_Ver']);
	if (b == 'percent') {
	    $('.info .gxcontent').attr('type', 'percent');

	    $('.info .gxcontent .left .left_rval').html(percentage(appState.info['H_POWER']) + '%');
	} else {
	    $('.info .gxcontent').attr('type', 'volts');

	    $('.info .gxcontent .left .left_rval').html((parseInt(appState.info['H_POWER']) / 100).toFixed(2) + 'V');

	    if (b == null) _setItem('battery', 'volts');
	}

	$('.info .gxcontent .right .right_lver').html(appState.info['L_Ver']);
	$('.info .gxcontent .right .right_rver').html(appState.info['R_Ver']);
	$('.info .gxcontent .right .right_lval').html((parseInt(appState.info['L_POWER']) / 100).toFixed(2) + 'V');
	$('.info .gxcontent .right .right_rval').html((parseInt(appState.info['R_POWER']) / 100).toFixed(2) + 'V');
    }
	...
	if ($(this).attr('type') == 'volts') {
	    $('.info .content .left .left_val').html(percentage(parseInt(appState.info['POWER_2'])) + '%');
	...
	//$('.info .content .left .left_ver').html(appState.info['GEARS_V']);
	if ($(this).attr('type') == 'volts') {
	    $('.info .ox2content .left .left_val').html(percentage(parseInt(appState.info['H_POWER'])) + '%');
	    $(this).attr('type', 'percent');
	} else {
	    $('.info .ox2content .left .left_val').html((parseInt(appState.info['H_POWER']) / 100).toFixed(2) + 'V');
	    $(this).attr('type', 'volts');
	...
	if ($(this).attr('type') == 'volts') {
	    $('.info .txcontent .left .left_fval').html(percentage(parseInt(appState.info['Q_POWER'])) + '%');
	    $('.info .txcontent .left .left_rval').html(percentage(parseInt(appState.info['H_POWER'])) + '%');
	    $(this).attr('type', 'percent');
	} else {
	    $('.info .txcontent .left .left_fval').html((parseInt(appState.info['Q_POWER']) / 100).toFixed(2) + 'V');
	    $('.info .txcontent .left .left_rval').html((parseInt(appState.info['H_POWER']) / 100).toFixed(2) + 'V');
	...
	if ($(this).attr('type') == 'volts') {
	    $('.info .gxcontent .left .left_rval').html(percentage(parseInt(appState.info['H_POWER'])) + '%');
	    $(this).attr('type', 'percent');
	} else {
	    $('.info .gxcontent .left .left_rval').html((parseInt(appState.info['H_POWER']) / 100).toFixed(2) + 'V');
	    $(this).attr('type', 'volts');
...
function _raceMode(f)
{
    ...
    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_setProtectionThreshold, 0x01, f, 0x00, 0x00 ]);
    a = setCRC16(a);
    characteristic_TX.writeValueWithoutResponse(a);
...
function _sleepMode(f)
{
    ...
    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_backDialSleep, 0x01, f, 0x00, 0x00 ]);
    a = setCRC16(a);
    characteristic_TX.writeValueWithoutResponse(a);
...
function _protectMode(f)
{
    ...
    var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd_backSetting2p, 0x08, f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 ]);
    a = setCRC16(a);
    characteristic_TX.writeValueWithoutResponse(a);
...
function qalert(s, close = 1)
{
    ...
    clearTimeout(appState.qtimeout);
    if (close) {
	appState.qtimeout = setTimeout(function() {
	    $('.qalert').fadeOut();
	}, 1500);
    }
}
...
export class BleClient extends EventTarget {
    ...
    async connect() {
        try {
            ...
            appState.device_type = device.name;
            store.setState({ deviceType: appState.device_type, connectionState: 'connected' });
            
            this.dispatchEvent(new CustomEvent('connected', { detail: appState.device_type }));
    ...
    sendPayload(cmd, payloadArray) {
        ...
        var a = new Uint8Array([ 0xfe, 0x32, appState.key, cmd, f, ...payloadArray, 0x00, 0x00 ]);
        a = setCRC16(a);
        characteristic_TX.writeValueWithoutResponse(a);
    }
